...
- (void)setScreen {
    ...
    CGRect scanFrame = CGRectMake(offset,offset+100,width - offset*2, width - offset*2);
...
- (void)setnoticeInfosetLabel{
    CGFloat width = CGRectGetWidth(self.frame);
    CGFloat offset = 60;
    _noticeInfoLable = [[UILabel alloc]initWithFrame:CGRectMake(offset, 90, width - offset*2 , 40)];
    _noticeInfoLable.numberOfLines = 0;
    [_noticeInfoLable setText:@"将二维码放入取景框中即可自动扫描"];
...
+ (UIImage *)scQRCodeForString:(NSString *)qrString size:(CGFloat)size{
   ...
   UIImage *imageReturn = [self createNonInterpolatedUIImageFormCIImage:outputImage withSize:size];
    return imageReturn;
}
